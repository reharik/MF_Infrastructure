...
module.exports = function(pgbluebird, Promise,uuid, logger, JSON){
    return function(options) {
        return {
            async getById(id, table){
                var pgb = new pgbluebird();
                try {
                    var cnn = pgb.connect(options.postgres.connectionString + options.postgres.methodFitness);
            ...
            async save(table, document, id){
                ...
                var result;
                try {
                    var cnn = await pgb.connect(options.postgres.connectionString + options.postgres.methodFitness);
                    if (id) {
            ...
            async checkIdempotency(originalPosition, eventHandlerName){
                var pgb = new pgbluebird();
                try {
                    var cnn = await pgb.connect(options.postgres.connectionString + postgres.methodFitness);
