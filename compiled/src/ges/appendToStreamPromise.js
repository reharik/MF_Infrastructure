...
module.exports = function appendToStreamPromise(Promise, invariant, logger, gesConnection) {
    return function (streamName, data) {
        ...
        invariant(data.expectedVersion, 'must pass data with an expected version of aggregate');
        invariant(data.events && data.events.length > 0, 'must pass data with at least one event');
